...
프로젝트에서 opencode가 UART MCP를 직접 사용하려면 루트의 `opencode.jsonc`를 참고한다. 기본 설정은 `tools/uart/targets.json`의 `sk` target을 사용하며, MCP tool 호출 시 `target: "custom"`처럼 target을 명시해 endpoint를 바꿀 수 있다.
