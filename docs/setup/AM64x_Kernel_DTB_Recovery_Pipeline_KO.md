...
Host PC local repo
  ...
  → Ethernet/SSH를 이용해 보드의 SD boot partition에 배포
  → SD card 기반 부팅으로 검증
```
...
```text
- 정상 Kernel/DTB 세트가 SD boot partition에 항상 존재해야 한다.
...
SD 내부의 정상 이미지 선택이 어렵거나, SD boot partition의 Kernel/DTB 파일 구성이 꼬인 경우를 대비해 TFTP 복구 흐름을 둔다.
...
6. Linux 진입 후 SD boot partition의 Kernel/DTB를 복구
```
...
현재 `U-Boot Ethernet/TFTP`는 가능하다고 판단하고, 파이프라인에는 복구 옵션으로 포함한다. 단, 실제 스크립트는 로컬 환경의 IP, TFTP root, U-Boot env 이름을 기준으로 작성한다.
...
Recovery 계층
  ...
  - TFTP Kernel/DTB recovery command 생성 또는 문서화
```
...
```text
1. Kernel + DTB 빌드
2. SD boot partition에 실험 Kernel/DTB 배포
3. SD boot로 부팅 검증
...
```text
- 현재 deploy script가 어느 파일을 SD boot partition에 쓰는지
